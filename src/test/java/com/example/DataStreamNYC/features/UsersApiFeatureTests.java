package com.example.DataStreamNYC.features;

import com.example.DataStreamNYC.models.User;
import com.example.DataStreamNYC.repositories.UserRepository;
import org.junit.After;
import org.junit.Before;
import org.junit.Test;
import org.junit.runner.RunWith;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.test.context.SpringBootTest;
import org.springframework.test.context.junit4.SpringRunner;

import java.util.stream.Stream;

import static io.restassured.RestAssured.given;
import static io.restassured.RestAssured.when;
import static io.restassured.http.ContentType.JSON;
import static org.hamcrest.Matchers.containsString;
import static org.hamcrest.Matchers.is;

@RunWith(SpringRunner.class)
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.DEFINED_PORT)
public class UsersApiFeatureTests {

    @Autowired
    private UserRepository userRepository;

    @Before
    public void setUp() {
        userRepository.deleteAll();
    }

    @After
    public void tearDown() {
        userRepository.deleteAll();
    }

    private User secondUser;

    @Test
    public void shouldAllowFullCrudForAUser() throws Exception {

        User firstUser = new User(
                "user1@email.com",
                "first1",
                "last1",
                "password1"
        );

        secondUser = new User(
                "user2@email.com",
                "first2",
                "last2",
                "password2"
        );

        Stream.of(firstUser, secondUser)
                .forEach(user -> {
                    userRepository.save(user);
                });

        // Test creating a User
        User userNotYetInDb = new User(
                "new@email.com",
                "new_first",
                "new_last",
                "newPassword"

        );

        given()
                .contentType(JSON)
                .and().body(userNotYetInDb)
            .when()
                .post("http://localhost:8080/api/users")
            .then()
                .statusCode(is(200))
                .and().body(containsString("new_first"));

        // Test get all Users
        when()
                .get("http://localhost:8080/api/users")
            .then()
                .statusCode(is(200))
                .and().body(containsString("first1"))
                .and().body(containsString("user2@email.com"))
                .and().body(containsString("new_last"))
                .and().body(containsString("password1"));

        // Test finding one user by ID
        when()
                .get("http://localhost:8080/api/users/" + secondUser.getId())
            .then()
                .statusCode(is(200))
                .and().body(containsString("first2"))
                .and().body(containsString("user2@email.com"));

        // Test updating a user
        secondUser.setFirstName("changed_firstName");

        given()
                .contentType(JSON)
                .and().body(secondUser)
            .when()
                .patch("http://localhost:8080/api/users/" + secondUser.getId())
            .then()
                .statusCode(is(200))
                .and().body(containsString("changed_firstName"));

        // Test deleting a user
        when()
                .delete("http://localhost:8080/api/users/" + secondUser.getId())
            .then()
                .statusCode(is(200));
    }
}